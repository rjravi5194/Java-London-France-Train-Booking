...
public class BookingServiceImpl implements BookingService {
    ...
    @Override
    public BookingDetailsDto modifyUserTicket(CustomerDetailsDao customerDetailsDao, String journeyDate,int seatNumber,String sectionType) {
        try {
            ...
            List<Integer> bookedSeats = ticketDetailsRepository.viewBookedSeats(customerDetailsDao.getSectionPreference(), journeyDate, customerDetailsDao.getSeatPreference().toString())
                    .stream().collect(ArrayList::new, (list, ticket) -> list.add(ticket.getSeatNumber()), ArrayList::addAll);
            if (bookedSeats.size() == 4) {
                ...
                Random rand = new Random();
                    seatNumber = seatMap.get(customerDetailsDao.getSeatPreference()).get(rand.nextInt(4));
                while (bookedSeats.contains(seatNumber)) {
                    seatNumber = seatMap.get(customerDetailsDao.getSeatPreference()).get(rand.nextInt(4));
                }
                ticketDetailsDao.setSeatNumber(seatNumber);
