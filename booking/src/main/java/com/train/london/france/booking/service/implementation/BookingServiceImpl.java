...
public class BookingServiceImpl implements BookingService {
    ...
    @Override
    public String removeUserFromTrainSection(String email, String journeyDate) {
        try {
            LOGGER.info(messageSource.getMessage("method.removeUserFromTrainSection", null, Locale.ENGLISH));
            int a = ticketDetailsRepository.removeUserFromTrainSection(email, journeyDate);
            if (a > 1) {
                return SUCCESS;
            } else {
                return FAILURE;
    ...
    @Override
    public BookingDetailsDto modifyUserTicket(CustomerDetailsDao customerDetailsDao, String journeyDate) {
        try {
            LOGGER.info(messageSource.getMessage("method.modifyUserTicket", null, Locale.ENGLISH));
            TicketDetailsDao ticketDetailsDao = ticketDetailsRepository.findByEmailAndJourneyDate(customerDetailsDao.getEmail(), journeyDate);
            ...
            List<Integer> bookedSeats = ticketDetailsRepository.viewBookedSeats(sectionType, journeyDate, customerDetailsDao.getSeatPreference().toString())
                    .stream()
                    .map(TicketDetailsDao::getSeatNumber).toList();
            if (bookedSeats.size() == 4) {
                ...
                Random rand = new Random();
                int seatNumber = seatMap.get(customerDetailsDao.getSeatPreference().toString()).get(rand.nextInt(4));
                while (bookedSeats.contains(seatNumber)) {
